<?php
/**
 * Core plugin class for Addon Manager.
 *
...
class Addon_Manager
{
    ...
    /**
     * Load active addons and apply safety guards before runtime include.
     *
     * @return void
     */
    public function load_active_addons()
    {
        ...
        if (!empty($core_active)) {
            foreach ($core_active as $relative_file) {
                $addon_path = plugin_dir_path( ADDON_MANAGER_FILE ) . $relative_file;
    ...
    private function get_addon_path_by_id($addon_id)
    {
        ...
        if ($parsed['origin'] === 'core') {
            return plugin_dir_path( ADDON_MANAGER_FILE ) . ltrim((string) $parsed['value'], '/');
        }
    ...
    /**
     * Return healthcheck JSON response during internal validation requests.
     *
     * @return void
     */
    ...
    private function get_available_core_relative_paths()
    {
        ...
        foreach ($this->get_core_folders_map() as $folder => $tab) {
            $addon_dir = plugin_dir_path( ADDON_MANAGER_FILE ) . $folder . '/';
            if (!is_dir($addon_dir)) {
                continue;
            }
    ...
    /**
     * Handle user addon uploads via admin-post.
     *
     * @return void
     */
    ...
    /**
     * Delete an uploaded user addon file.
     *
     * @return void
     */
    ...
    /**
     * Keep backward compatibility for upload AJAX requests.
     *
     * @return void
     */
    ...
    /**
     * Prepare payload for internal healthcheck requests.
     *
     * @return void
     */
    ...
    private function get_detected_settings_pages()
    {
        ...
        $plugin_base = wp_normalize_path(plugin_dir_path(ADDON_MANAGER_FILE));
    ...
    /**
     * Enqueue admin assets only on Addon Manager page.
     *
    ...
    {
        if ($hook !== 'toplevel_page_addon-manager') {
            return;
        }
    ...
    /**
     * Render Addon Manager admin interface.
     *
     * @return void
     */
...
<?php
    ...
    /**
     * Toggle addon activation state via AJAX.
     *
     * @return void
     */
